...
async def process_parallel_candidates(data_list, exp_mode="dev_planner_critic", retrieval_setting="auto", model_name="", image_model_name="", provider="evolink", api_key="", base_url=""):
    ...
    if api_key:
        ...
        if provider == "evolink":
            ...
            # 初始化自定义 OpenAI 兼容 provider
            generation_utils.init_custom_openai_provider(api_key, base_url)
    else:
    ...
    try:
        ...
        if generation_utils.custom_openai_provider and hasattr(generation_utils.custom_openai_provider, 'close'):
            await generation_utils.custom_openai_provider.close()
    ...
    with tab1:
        ...
        with st.sidebar:
            ...
            provider = st.selectbox(
                "API Provider",
                ["gemini", "evolink", "custom_openai"],
                index=0,
                key="tab1_provider",
                help="gemini：Google 官方 API（需翻墙）| evolink：国内代理 | custom_openai：自定义 OpenAI 兼容 API"
            )
            ...
            _provider_defaults = {
                ...
                "custom_openai": {
                    "api_key_label": "API Key",
                    "api_key_help": "OpenAI 兼容 API 密钥",
                    "api_key_default": get_config_val("custom_openai", "api_key", "CUSTOM_OPENAI_API_KEY", ""),
                    "model_name": "gpt-4",
                    "image_model_name": "dall-e-3",
                    "show_base_url": True,
                    "base_url_default": get_config_val("custom_openai", "base_url", "CUSTOM_OPENAI_BASE_URL", "https://api.openai.com"),
                },
            }
            ...
            if "tab1_base_url" not in st.session_state:
                st.session_state["tab1_base_url"] = _pd.get("base_url_default", "")
            ...
            if st.session_state["prev_provider"] != provider:
                ...
                st.session_state["tab1_image_model_name"] = _pd["image_model_name"]
                st.session_state["tab1_api_key"] = _pd["api_key_default"]
                if _pd.get("show_base_url"):
                    st.session_state["tab1_base_url"] = _pd.get("base_url_default", "")
                st.rerun()

            # Base URL（仅对自定义 OpenAI 显示）
            base_url = ""
            if _pd.get("show_base_url"):
                base_url = st.text_input(
                    "Base URL",
                    key="tab1_base_url",
                    help="API 服务的基础 URL，例如：https://api.openai.com 或 https://your-custom-api.com",
