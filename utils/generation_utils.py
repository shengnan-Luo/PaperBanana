...
# 导入自定义 OpenAI 兼容 Provider 支持
try:
    from .custom_openai_utils import (
        init_custom_openai_provider,
        call_custom_openai_text_with_retry_async,
        call_custom_openai_image_with_retry_async,
        custom_openai_provider
    )
except ImportError as e:
    print(f"警告：无法导入自定义 OpenAI 兼容 Provider 支持: {e}")

    # 提供备用函数
    def init_custom_openai_provider(api_key: str, base_url: str = "https://api.openai.com"):
        print("警告：自定义 OpenAI 兼容 Provider 不可用")

    async def call_custom_openai_text_with_retry_async(*args, **kwargs):
        raise RuntimeError("自定义 OpenAI 兼容 Provider 不可用")

    async def call_custom_openai_image_with_retry_async(*args, **kwargs):
        raise RuntimeError("自定义 OpenAI 兼容 Provider 不可用")
